...
**(i)** In the first provided chunk of code, we start by initializing two variables x and z with 100 values of a normal distribution and then, although uncorrelated, fit a linear model of x on z. We then apply the sinus function to the residuals of the model, add a little offset, update x with it and refit the model. This step is repeated 4 times. To prevent x from holding only small values, a threshold is introduced and checked in each iteration. In the end, the transformed x vector is returned.
...
-   In both code chunks, large parts are repetitive and could be replaced by a loop. In particular we could use a loop for the residual computation and fitting process in (i) and for the train-predict-evaluate procedure in (ii).
...
lm_cv_4fold <- function(){
  ...
  for(i in 1:4){
    # test set is the i-th quarter
    test <- ((i-1)*250+1):(i*250)
...
For this task, we were given the following implementation of a Kruskal-Wallis rank sum test in R:
...
```{r}
group1 <- c(8.1, 8.5, 8.3)
group2 <- c(9.2, 9.4, 9.1)
group3 <- c(8.9, 9.0, 9.1)
...
```{r}
x <- c(8.1, 8.5, 8.3, 9.2, 9.4, 9.1, 8.9, 9.0, 9.1)
...
As expected/necessary, both calls return the same result: A p value of 0.032, indicating that the null hypothesis of equal medians across groups can be rejected at the 0.05 significance level.


### c) Refactored Code (Performance Improvements)

Here we refactor the code, trying to increase performance, by restricting the input to be a numeric variable as x and its group membership as g. This way, we can reduce the number of checks and conversions needed.
...
Again, we get a p-value of 0.032, indicating that the refactored function works correctly. 


### d) Matrix version for original kwtest function

Here, we write a wrapper function around the original implementation, to make it usable for multiple repetitions of the test. 
...
repeated_kwtest(X, grp)[1:10] # only show first 10 results as example
```


### e) Matrix version for optimized kwtest function

We do the same for the optimized version of the function.
...
repeated_kwtest_opt(X, grp)[1:10] # only show first 10 results as example
```

### f) Comparison via benchmark

To compare the runtimes of the original and optimized versions, we use the `microbenchmark` package.
...
# Plot the results
autoplot(runtime) + ggtitle("Benchmark for original kwtest function")
autoplot(runtime_opt) + ggtitle("Benchmark for optimized kwtest function")
```

The results show, that while the methods have a roughly equal median, we get differences in the means. For the original function we get a mean runtime of 681 ms, which the optimized version is able to decrease to 634 ms. Although, this reduction of around 7 % is not very large, it is still a significant improvement. Also we note from the plots, that the optimized version has fewer worst-case/very slow occurences, while the original version has a long tail up to over 900 ms.


### g) Vectorized version of kwtest function

Lastly, we try to increase performance even further by vectorizing the function. We do so by exchanging the `tapply` calls with `rowsum` and `table` calls.
...
repeated_kwtest_vec <- function(X, g) {
  res <- apply(X, 1, function(x) {
    stat <- kwtest_vec(x, g)
    return(stat$statistic)
  }
...
autoplot(runtime_vec) + ggtitle("Benchmark for vectorized kwtest function")
```

Against our expectations, the vectorized version is slower than the optimized version. The mean runtime is around 900 ms, which is significantly slower than the optimized and original version. 

We think that one reason for this could be the calls to `rowsum` and `table`. As the example is rather small, the overhead of these functions might be more significant than the gain from vectorization.

With a large number of groups, the vectorized version might be more efficient, as it avoids the repeated calculation of the rank sums for each group. However, in our case, the optimized version is the fastest.